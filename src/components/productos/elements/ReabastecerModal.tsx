...
export default function ReabastecerModal({ open, onClose, producto, productosLista = [], onSuccess }: Props) {
  ...
  const [lotesExistentes, setLotesExistentes] = useState<Array<{ id: string; numero_lote: string; fecha_vencimiento: string | null; stock_actual: number }>>([]);
  ...
  const productoSeleccionado = producto || productosLista.find((item) => item.producto_comercial_id === prodId);
  const controlaLote = productoSeleccionado?.controla_lote ?? true;
  const requiereVencimiento = productoSeleccionado?.requiere_vencimiento ?? true;
  const esReabastecimientoDeLote = Boolean(producto) && controlaLote;
  const presentaciones = useMemo(() => {
    const delListado = productosLista.filter(p => p.producto_comercial_id === prodId && p.presentacion_id);
    return delListado.length > 0 ? delListado : presentacionesDetalle;
  }, [productosLista, presentacionesDetalle, prodId]);
  ...
  useEffect(() => {
    ...
    productosService.getProductoDetalle(prodId)
      .then((detalle) => {
        ...
        setPresentacionesDetalle((detalle.presentaciones || []).map((item: any) => ({
          producto_comercial_id: prodId,
          nombre_comercial: detalle.nombre_comercial,
          sku: detalle.sku,
          presentacion_id: item.id,
          presentacion_nombre: item.unidad_presentacion?.nombre || "Presentación",
          codigo_barras: item.codigo_barras || undefined,
          cantidad_unidad_base: Number(item.cantidad_unidad_base || 1),
          controla_lote: detalle.controla_lote,
          requiere_vencimiento: detalle.requiere_vencimiento,
        })));
      })
      .catch(() => {
        if (!activo) return;
        setLotesExistentes([]);
        setPresentacionesDetalle([]);
      });
    return () => { activo = false; };
  }, [open, prodId]);
